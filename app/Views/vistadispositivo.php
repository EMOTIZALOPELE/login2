...
<<<<<<< HEAD
          ...
          <div class="row">
            ...
            <div class="col-lg-6 mt-5 mt-lg-0">
                ...
                <h3 style="color: var(--danger-color);">
                  <i class="fas fa-exclamation-triangle"></i> Eliminación de Horarios
                </h3>
                <h4 style="color: var(--danger-color);">Eliminar Tarjeta de Horarios y Servos</h4>
                <p style="color: var(--text-secondary);">
                  **Esta acción es irreversible.** Eliminará permanentemente la tarjeta de horarios, todos sus servos asociados y sus días de operación.
                </p>
                <p style="color: var(--accent-color); font-size: 0.9rem;">
                  <i class="fas fa-info-circle"></i> **El dispositivo principal se mantendrá en tu cuenta** y podrás asignarle una nueva tarjeta de horarios.
                </p>
                <?php if ($item['tarjeta']): ?>
                  <button type="button" class="btn-custom btn-danger-custom" 
                      data-toggle="modal" 
                      data-target="#confirmDeleteModal"
                      data-card-id="<?= esc($item['tarjeta']['idhorario']) ?>"
                      data-card-name="<?= esc($item['tarjeta']['nombre_tarjeta']) ?>"
                      data-device-name="<?= esc($item['dispositivo']['nombre_dispositivo'] ?? 'Dispositivo ' . $item['dispositivo']['id']) ?>">
                    <i class="fas fa-times-circle"></i> Eliminar Tarjeta y Servos
                  </button>
                <?php else: ?>
                  <div class="alert alert-info">
                    No hay una tarjeta de horarios asociada para eliminar.
                  </div>
                <?php endif; ?>
              </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="confirmDeleteModalLabel">Confirmar Eliminación de Tarjeta</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <p>¿Estás seguro de que quieres eliminar la tarjeta de horarios "<span id="cardNameToDelete" class="font-weight-bold"></span>"?</p>
          <p class="font-weight-bold" style="color: var(--danger-color);">
            Atención: Esta acción ejecutará la eliminación en cascada: se borrará la Tarjeta y **todos los Servos** y sus registros de días asociados.
          </p>
          <p>El dispositivo "<span id="deviceNameToDelete" class="font-weight-bold"></span>" se **mantendrá** en tu cuenta, pero sin horarios ni Servos asociados. Esta acción no se puede deshacer.</p>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
          <form id="deleteDeviceForm" action="" method="POST" style="display: inline;">
            <?= csrf_field() ?>
            <button type="submit" class="btn btn-danger">Sí, Eliminar Tarjeta y Servos</button>
          </form>
        </div>
      </div>
    </div>
  ...
  $(document).ready(function() {
    // Script para manejar el modal de confirmación de borrado (AHORA SOLO PARA ELIMINAR TARJETA/SERVOS)
    $('#confirmDeleteModal').on('show.bs.modal', function (event) {
      var button = $(event.relatedTarget);
      // Capturamos el ID de la tarjeta, no el action del dispositivo
      var cardId = button.data('card-id'); 
      var cardName = button.data('card-name');
      var deviceName = button.data('device-name');

      // Construimos la acción del formulario para que apunte a la eliminación de la tarjeta
      // ASUME que la ruta 'horarios/borrar-tarjeta/{id}' está definida en Routes.php
      var formAction = '<?= site_url('horarios/borrar-tarjeta/') ?>' + cardId;
